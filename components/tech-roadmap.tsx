...
const initialTechNodes: TechNode[] = [
  {
    ...
    category: "languages",
    level: 1,
    description: "Type-safe JavaScript - the foundation of my development approach",
    connections: ["react", "nodejs", "nestjs", "express"],
    x: 50,
    y: 10,
  },
  {
  ...
  },
  {
    ...
    connections: ["express", "nestjs", "typescript"],
    x: 20,
    y: 50,
  },
  {
    ...
    connections: ["nodejs", "typescript", "aws"],
    x: 10,
    y: 65,
  },
  {
  ...
  },
  {
    ...
    category: "languages",
    level: 5,
    description: "Systems programming language - memory safe and performant",
    connections: ["nodejs", "aws"],
    x: 65,
    y: 50,
  },
  {
    ...
    connections: ["nodejs", "express", "nestjs", "rust"],
    x: 75,
    y: 75,
  },
]
...
export function TechRoadmap() {
  ...
  const [visitedNodes, setVisitedNodes] = useState<Set<string>>(new Set(["typescript"]))
  const [hoveredNode, setHoveredNode] = useState<string | null>(null)
  const [techNodes, setTechNodes] = useState<TechNode[]>(initialTechNodes)
  const [draggedNode, setDraggedNode] = useState<string | null>(null)
  const [dragOffset, setDragOffset] = useState({ x: 0, y: 0 })
  const [selectedCategory, setSelectedCategory] = useState<"frontend" | "backend" | "languages" | "tools" | null>(null)
  ...
  const filteredNodes = selectedCategory
    ? techNodes.filter((node) => node.category === selectedCategory || visitedNodes.has(node.id))
    : techNodes
  ...
  const isAccessible = (nodeId: string) => {
    return availableNodes.includes(nodeId) || visitedNodes.has(nodeId) || nodeId === currentNode
  }
    ...
    <section id="roadmap" className="min-h-screen flex items-center justify-center px-6 py-20 relative overflow-hidden">
      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        <div className="text-center mb-12 space-y-4">
          <h2 className="text-5xl md:text-7xl font-bold bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 bg-clip-text text-transparent animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <p className="text-xl md:text-2xl text-muted-foreground animate-in fade-in slide-in-from-bottom-5 duration-700 delay-100">
            Navigate through my tech stack • Click to explore • Drag to rearrange
          </p>
        </div>
        ...
        <div className="relative w-full aspect-[16/10] bg-gradient-to-br from-background via-background to-muted/20 border-2 border-primary/20 rounded-2xl overflow-hidden mb-8 shadow-2xl shadow-primary/10">
          ...
          >
            <defs>
              <linearGradient id="gradient-frontend" x1="0%" y1="0%" x2="100%" y2="100%">
              ...
              <linearGradient id="gradient-languages" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" stopColor="#f97316" />
                <stop offset="50%" stopColor="#f59e0b" />
                <stop offset="100%" stopColor="#eab308" />
              </linearGradient>
              <linearGradient id="gradient-tools" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            {techNodes.map((node) =>
              node.connections.map((connId) => {
                ...
                const isActive = visitedNodes.has(node.id) && visitedNodes.has(connId)
                const isAvailable = currentNode === node.id && availableNodes.includes(connId)
                const isFiltered =
                  !selectedCategory || (filteredNodes.includes(node) && filteredNodes.includes(connectedNode))
                ...
                return (
                  <path
                    ...
                    fill="none"
                    stroke={isActive || isAvailable ? getConnectionColor(node.category) : "hsl(var(--border))"}
                    strokeWidth={isActive ? "0.5" : isAvailable ? "0.4" : "0.2"}
                    opacity={isFiltered ? (isActive ? "0.8" : isAvailable ? "0.6" : "0.3") : "0.1"}
            ...
            {filteredNodes.map((node) => {
              ...
              return (
                <g key={node.id} className={isDragging ? "cursor-grabbing" : "cursor-grab"}>
                  {isCurrent && (
                    <>
                      <circle
                        cx={node.x}
                  ...
                  <circle
                    cx={node.x}
                    cy={node.y}
                    r={isCurrent ? "2.8" : isHovered || isDragging ? "2.3" : "1.8"}
        ...
        {currentTech && (
          <Card className="p-6 animate-in fade-in slide-in-from-bottom-4 duration-300 border-2 bg-gradient-to-br from-card to-card/50 backdrop-blur-sm shadow-xl">
            <div className="flex items-start justify-between mb-4">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <Badge
                    className={`bg-gradient-to-r ${categoryColors[currentTech.category]} text-white border-0 shadow-lg ${categoryGlowColors[currentTech.category]}`}
                  >
            ...
            <p className="text-muted-foreground mb-6 leading-relaxed">{currentTech.description}</p>
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {availableNodes.map((connId) => {
                    const connectedTech = techNodes.find((n) => n.id === connId)
            ...
            {visitedNodes.size > 1 && (
              <div className="mt-6 pt-6 border-t">
                <p className="text-sm font-semibold mb-3">Journey so far ({visitedNodes.size} technologies):</p>
                <div className="flex flex-wrap gap-2">
